...
class MeristemH5Dataset(Dataset):
    ...
    def __init__(self, list_path, data_root='', patch_size=(64,128,128), norm_method='percentile', shuffle=True,\
        ...
        for i in range(num_files):
            ...
            if not self.no_mask:
                for num_mask in range(test_sample['mask'].shape[0]):
                    io.imsave(os.path.join(test_folder, 'test_mask{0}_group{1}.tif'.format(i,num_mask)), test_sample['mask'][num_mask,...])
